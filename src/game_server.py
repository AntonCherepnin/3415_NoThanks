...
class GameServer:
    ...
    def new_game(cls):
        ...
        game_state = GameState(list(player_types.keys()), deck, card_top) 
    ...
    def bidding_phase(self)  -> GamePhase:
        ...
        if PlayerInteraction.choose_action() == 'Take card':
            ...
            PlayerInteraction.inform_card_take()
            return GamePhase.START_BIDDING
        elif PlayerInteraction.choose_action() == 'Spend':
            self.game_state._raise()
            PlayerInteraction.inform_player_spend()
